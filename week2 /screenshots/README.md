...
The screenshots below are listed in the same order they appear in the **Week 2 / Screenshot** folder itself.

| # | Filename | What It Covers |
|---|---|---|
| 1 | `const_screenshot.png` | Defining a constant and comparing a variable with `if`/`else` |
| 2 | `do while loop.png` | Running a loop body at least once with `do...while` |
| 3 | `if.png` | Grading marks with `if`, `elseif`, and `else` |
| 4 | `loop control.png` | Counting up to 5 with a `while` loop |
| 5 | `loop.png` | Building a 12-times table with a `for` loop |
| 6 | `nested loop.png` | A loop inside a loop, building a small multiplication table |
| 7 | `switch statement.png` | Grading marks again, this time with a `switch` statement |
| 8 | `ternary operator.png` | A one-line if/else using the ternary operator |

---

## 1. `const_screenshot.png` — Constants and a Simple Comparison

![Constants Screenshot](const_screenshot.png)
...
## 2. `do while loop.png` — Running the Loop Body at Least Once

![Do While Loop Screenshot](do while loop.png)

**What's in the image:** the `// do while loop` block, which repeats a 1-to-5 counting pattern using a `do...while` loop instead of an ordinary `while` loop.

**Detailed breakdown:**

- **Setting the counter:** `$count = 1;` initializes the variable the loop will count with.
...
- **What happens inside the loop:** `echo $count . "<br>"; $count++;` prints the current value of `$count` followed by a line break, then increases `$count` by `1` before the condition is checked again. This produces the numbers 1 through 5, each on its own line.
...
**Why this matters:** The "check-first vs. check-last" distinction between `while` and `do...while` matters for situations where a loop body genuinely must run once regardless of the condition — for example, showing a menu before ever asking whether to repeat it.

---

## 3. `if.png` — Grading Marks with `if` / `elseif` / `else`

![If Else Screenshot](if.png)
...
![While Loop Screenshot](loop control.png)
...
## 5. `loop.png` — Building a 12-Times Table with `for`

![For Loop Screenshot](loop.png)
...
## 6. `nested loop.png` — A Loop Inside a Loop

![Nested Loop Screenshot](nested loop.png)
...
## 7. `switch statement.png` — Grading Marks Again, with `switch`

![Switch Statement Screenshot](switch statement.png)

**What's in the image:** the `//Marks grade using switch` block, which re-solves the same kind of grading problem as the earlier `if`/`elseif` screenshot, but using a `switch` statement instead.

**Detailed breakdown:**

- **Setting up the value again:** `$marks = 100;` gives a fresh mark to grade, separate from the `87` used in the `if`/`elseif` example.
- **A `switch` built on boolean expressions, not fixed values:** A `switch` statement is normally used to compare a variable against a list of fixed values (`case 1:`, `case 2:`, etc.). This code instead writes full boolean expressions as the `case` labels — `case($marks >= 90):`, `case($marks >= 85):`, and so on. This works because PHP's `switch` uses loose comparison, so a `case` whose expression evaluates to a matching result can still be triggered. In practice, this produces the same top-to-bottom, "first match wins" behavior as the `if`/`elseif` chain.
...
- **The `default` case:** `default: echo "Fail"; break;` runs only if none of the `case` conditions match, serving the same role as the `else` in the `if`/`elseif` version.
...
**Why this matters:** Comparing this to the `if`/`elseif` screenshot makes the relationship explicit — both blocks solve the same grading problem, showing two different tools that can express the same branching logic, with `switch` reading a little differently but still relying on the same "first match wins, then stop" behavior once conditions are involved.

---

## 8. `ternary operator.png` — A One-Line Conditional

![Ternary Operator Screenshot](ternary operator.png)
...
Each screenshot above corresponds to its matching numbered section in this README, listed in the same order they appear in the **Week 2 / Screenshot** folder. The underlying source file, `test.php`, lives in the parent `Week 2` folder.
